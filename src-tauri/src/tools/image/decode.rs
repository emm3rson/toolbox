...
/// Largest allowed side length (px) before a source is rejected as too large.
const MAX_DIMENSION: u32 = 16_384;

/// Largest allowed pixel count (width × height) before a source is rejected,
/// guarding peak decode memory (bounded by the batch's concurrency).
...
mod tests {
  ...
  fn over_limit_dimension_is_rejected() {
    ...
    assert!(matches!(err, crate::errors::ProcessingError::InvalidImage { .. }));
    std::fs::remove_dir_all(&dir).unwrap();
  }
