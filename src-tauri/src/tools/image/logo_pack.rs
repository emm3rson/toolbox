...
/// Minimum square source resolution for the Logo Pack (matches the UI hint
...
/// Generates the selected Standard Web Pack assets from a square source into a
...
/// validation failures (non-square, too small, unsupported) return `Err`.
///
...
pub fn generate(
  ...
  let source = decode::decode(source_path)?;
...
fn generate_asset(source: &image::DynamicImage, output_dir: &Path, asset_id: &str) -> FileResult {
...
fn generate_asset_inner(
  ...
  let bytes = if preset.format == "ico" {
    ...
    let img = resize::resize(source, (preset.width, preset.height));
  ...
  Ok((output_path.to_string_lossy().into_owned(), bytes.len() as u64))
}

/// Encodes a multi-size ICO: each size is resized from the source, PNG-encoded,
/// and packed into one ICO via `IcoFrame`/`IcoEncoder` (image 0.25 API).
fn encode_ico(source: &image::DynamicImage) -> Result<Vec<u8>, ProcessingError> {
  let mut frames = Vec::with_capacity(ICO_SIZES.len());
  for &size in ICO_SIZES {
    let img = resize::resize(source, (size, size));
...
mod tests {
  ...
  use super::{STANDARD_WEB_PACK, generate, preset_by_id};
  use crate::errors::ProcessingError;
