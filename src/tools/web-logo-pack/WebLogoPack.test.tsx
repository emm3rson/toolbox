...
describe('WebLogoPack', () => {
  ...
  it('accepts a small SVG source (< 512px) without rejecting for size', async () => {
    const user = userEvent.setup()
    mockTauri.pickFiles.mockResolvedValue(['C:\\icon.svg'])
    mockTauri.inspectFiles.mockResolvedValue([
      source({ path: 'C:\\icon.svg', name: 'icon.svg', extension: 'svg', width: 24, height: 24 }),
    ])
    renderWithProviders(<WebLogoPack />)
  ...
  it('accepts a non-square SVG source without rejecting for aspect ratio', async () => {
    const user = userEvent.setup()
    mockTauri.pickFiles.mockResolvedValue(['C:\\banner.svg'])
    mockTauri.inspectFiles.mockResolvedValue([
      source({ path: 'C:\\banner.svg', name: 'banner.svg', extension: 'svg', width: 200, height: 100 }),
    ])
    renderWithProviders(<WebLogoPack />)
    ...
    expect(await screen.findByText('Standard Web Pack')).toBeInTheDocument()
    expect(screen.getByText('banner.svg')).toBeInTheDocument()
  })
})
