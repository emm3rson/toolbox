...
export function WebLogoPack() {
  const { exportPath, chooseExportPath } = useSettings(); const [phase,setPhase] = useState<Phase>('empty'); const [source,setSource] = useState<InputFile>(); const [invalidReason,setInvalidReason] = useState<InvalidReason>('non-square'); const [presets,setPresets] = useState<LogoAssetDefinition[]>(FALLBACK_PRESETS); const [assets,setAssets] = useState<AssetRow[]>(() => FALLBACK_PRESETS.map(toAssetRow)); const [progress,setProgress] = useState(0); const [result,setResult] = useState<GenerateLogoPackResult>(); const [error,setError] = useState<string>(); const [snippetOpen,setSnippetOpen] = useState(false); const [copied,setCopied] = useState(false)
  const selected = assets.filter((asset) => asset.on)
  useEffect(() => {
    desktop.getLogoPresets().then((list) => { if (list.length > 0) { setPresets(list); setAssets(list.map(toAssetRow)) } }).catch(() => {})
  }, [])
  ...
  const reset = () => { setPhase('empty'); setSource(undefined); setAssets(presets.map(toAssetRow)); setProgress(0); setResult(undefined); setError(undefined) }
