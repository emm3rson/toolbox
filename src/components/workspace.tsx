...
export const isVideoFile = (file: InputFile) =>
  VIDEO_EXTENSIONS.includes(file.extension.toLowerCase())
...
export function FileRow({
  ...
  const isImageValid = file.status === 'ready' && !loadError && !isPdf && !isVideo
...
export function ResizePanel({
  ...
  useEffect(() => {
    if (sourceSize?.width && sourceSize?.height) {
      setWidth(sourceSize.width)
      setHeight(sourceSize.height)
    }
  ...
  return (
    <div className="flex items-center gap-3 rounded-[var(--radius)] border border-border bg-card/60 px-3.5 py-2.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <span className="block truncate font-mono text-[12.5px]" title={path}>
