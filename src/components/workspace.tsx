...
export function SectionLabel({ children, hint }: { children: ReactNode; hint?: ReactNode }) { return <div className="flex items-baseline justify-between mb-2.5"><h3 className="text-[11px] font-semibold uppercase tracking-[.09em] text-subtle-foreground">{children}</h3>{hint}</div> }

export function DropZone({ onAdd, hint }: { onAdd: (paths?: string[]) => void; hint?: ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      unlisten = getCurrentWebview().onDragDropEvent((event) => {
        if (event.payload.type === 'enter' || event.payload.type === 'over') setOver(true)
        else if (event.payload.type === 'leave') setOver(false)
        else if (event.payload.type === 'drop') { setOver(false); onAddRef.current(event.payload.paths) }
      })
    } catch { return }
    return () => { if (unlisten) void unlisten.then((fn) => fn()) }
  }, [])
  return <div><div tabIndex={0} role="button" onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onAdd() }} onClick={() => onAdd()} className={cx('group relative rounded-[var(--radius-lg)] border border-dashed cursor-pointer flex flex-col items-center justify-center text-center transition-colors duration-200 py-16 px-6 outline-none focus-visible:ring-2 focus-visible:ring-ring/50', over ? 'border-ring bg-muted/70 ring-4 ring-ring/10' : 'border-border-strong bg-card/50 hover:bg-muted/40 hover:border-ring/50')}><div className={cx('mb-3.5 grid place-items-center h-11 w-11 rounded-full', over ? 'bg-foreground text-background' : 'bg-muted text-muted-foreground')}><UploadIcon size={20}/></div><p className="text-[15px] font-medium text-foreground">{over ? 'Drop to add files' : 'Drop images here'}</p><p className="text-[13px] text-muted-foreground mt-1">or <span className="text-foreground underline decoration-border-strong underline-offset-2">browse files</span></p></div>{hint && <p className="mt-3 text-[13px] text-muted-foreground leading-relaxed">{hint}</p>}</div>
}

export type RowStatus = 'idle' | 'processing' | 'done' | 'failed'
export function FileRow({ file, status = 'idle', onRemove, error }: { file: InputFile; status?: RowStatus; onRemove?: () => void; error?: string }) {
  return <div className={cx('flex items-center gap-3 px-3 py-2.5 rounded-[var(--radius-sm)] transition-colors', status === 'failed' ? 'bg-danger-surface' : 'hover:bg-muted/60')}><div className="relative h-9 w-9 shrink-0 rounded-[6px] bg-muted grid place-items-center text-muted-foreground overflow-hidden"><ImageIcon size={17}/><span className="absolute bottom-0 inset-x-0 text-[7.5px] font-semibold uppercase text-center bg-foreground/72 text-background leading-[10px]">{file.extension}</span></div><div className="min-w-0 flex-1"><p className="truncate text-[13.5px] font-[450] text-foreground">{file.name}</p><p className="font-mono text-[11.5px] text-muted-foreground mt-0.5 tabular-nums">{file.width > 0 && file.height > 0 ? <>{file.width.toLocaleString()} × {file.height.toLocaleString()}<span className="text-border-strong mx-1.5">·</span></> : null}{formatBytes(file.size)}{error && <span className="text-danger ml-1.5 font-sans">— {error}</span>}</p></div>{status === 'idle' && onRemove && <button onClick={onRemove} aria-label={`Remove ${file.name}`} className="h-7 w-7 grid place-items-center rounded-[6px] text-subtle-foreground hover:text-danger hover:bg-danger-surface"><XIcon size={15}/></button>}{status === 'processing' && <SpinnerIcon size={16} className="animate-spin text-muted-foreground"/>}{status === 'done' && <CheckIcon size={16} className="text-success"/>}{status === 'failed' && <AlertIcon size={16} className="text-danger"/>}</div>
}
...
export function ExportLocation({ path, onChange }: { path: string; onChange: () => void }) { return <div className="flex items-center gap-3 rounded-[var(--radius)] border border-border bg-card/60 px-3.5 py-2.5"><FolderIcon size={17} className="text-muted-foreground shrink-0"/><div className="min-w-0 flex-1"><span className="block text-[11px] font-medium uppercase tracking-[.07em] text-subtle-foreground">Export to</span><span className="block truncate font-mono text-[12.5px]">{path}</span></div><Button size="sm" variant="ghost" onClick={onChange}>Change</Button></div> }
